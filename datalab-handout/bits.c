...
int isLess(int x, int y) {
    int sign_x = (x>>31) & 1;
    int sign_y = (y>>31) & 1;
    int same_sign = !(sign_x ^ sign_y) & (((x + ~y + 1) >> 31) & 1);
...
unsigned float_i2f(int x) {
    ...
    unsigned tmp, i, E, exp, frac, round, mask, half;
    unsigned result = 0;
    if (x) 
    {
        if (sign)
        ...
        if (E < 24)
            ...
            round = (mask & x);
            half = 1<<(E-24);
            if ((round > half) || ((round == half) && ((frac & 1) == 1)))
                frac += 1;
        }
