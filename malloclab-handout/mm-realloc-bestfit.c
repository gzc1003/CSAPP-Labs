...
void *realloc(void *oldptr, size_t size)
{
    ...
    new_size = round_up(max(size + DSIZE, MIN_BLOCK_SIZE), ALIGNMENT);

    oldblock = payload_to_header(oldptr);
    old_size = get_size(oldblock->header);

    if (old_size >= new_size) {
        place(oldblock, new_size);
        newptr = header_to_payload(oldblock);
        return newptr;
    }

    block_t *block_next = find_next(oldblock);
    block_t *block_prev = find_prev(oldblock);

    bool prev_alloc = get_alloc(block_prev->header);
    bool next_alloc = get_alloc(block_next->header);

    size_t prev_size = prev_alloc ? 0 : get_size(block_prev->header);
    size_t next_size = next_alloc ? 0 : get_size(block_next->header);

    if (prev_size + old_size + next_size < new_size) {
        newptr = malloc(size);
    ...
    } else if (old_size + next_size > new_size) {
        remove_block(block_next);
        write_header(oldblock, old_size + next_size, 0);
        write_footer(oldblock);
        place(oldblock, new_size);
        newptr = header_to_payload(oldblock);
    } else if (prev_size + old_size + next_size > new_size) {
        block = coalesce(oldblock);
        newptr = header_to_payload(block);
        memmove(newptr, oldptr, old_size - DSIZE);
        place(block, new_size);
    }
    return newptr;
}
...
{
//    dbg_printf("insert %p\n", block);
    size_t size = get_size(block->header), curr_size;

    int i = find_free_list(size);

    block_t *free_listp = free_list_array[i];

    block_t *curr_block, *prev_block, *tmp;
...
//        dbg_printf("next block %p\n", block->next);
        for (curr_block = free_listp; curr_block != NULL;
             curr_block = find_free_next(curr_block)) {
            curr_size = get_size(curr_block->header);
            tmp = curr_block;
//            mm_checkheap(__LINE__);
            if (curr_size >= size) {
                prev_block = find_free_prev(curr_block);
                if (!prev_block)
...
//            dbg_printf("current block %p\n", curr_block);
//            dbg_printf("next block %p\n", find_free_next(curr_block));
//            dbg_printf("cannot break \n");
        }
